...
peer validation for similar hardware/arisen-version/config will be <= m
...
#       * image will be rendered by gatsby-remark-graphviz plugin in arisen docs.
...
Running wasm-runtime=rsn-vm-jit rsn-vm-oc-enable on relay node will reduce the validation time.
