...
The `chain_plugin` is a core plugin required to process and aggregate chain data on an ARISEN node.
...
Config Options for arisen::chain_plugin:
  ...
  --wasm-runtime rsn-vm|rsn-vm-jit      Override default WASM runtime (wabt)
  --rsn-vm-oc-enable                    Enable optimized compilation in WASM
