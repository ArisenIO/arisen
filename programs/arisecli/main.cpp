...
#include <arisen/version/version.hpp>
...
struct set_account_permission_subcommand {
   ...
   set_account_permission_subcommand(CLI::App* accountCmd) {
      ...
      permissions->add_flag("--add-code", add_code, localized("[code] add '${code}' permission to specified permission authority", ("code", name(config::arisen_code_name))));
      permissions->add_flag("--remove-code", remove_code, localized("[code] remove '${code}' permission from specified permission authority", ("code", name(config::arisen_code_name))));
      ...
      permissions->set_callback([this] {
         ...
         if ( need_parent || need_auth ) {
            ...
            if ( need_auth ) {
               ...
               auto code_name = config::arisen_code_name;
...
struct deposit_subcommand {
   ...
   deposit_subcommand(CLI::App* actionRoot) {
      auto deposit = actionRoot->add_subcommand("deposit", localized("Deposit into owner's COM fund by transfering from owner's liquid token balance"));
      deposit->add_option("owner",  owner_str,  localized("Account which owns the COM fund"))->required();
      deposit->add_option("amount", amount_str, localized("Amount to be deposited into COM fund"))->required();
...
struct withdraw_subcommand {
   ...
   withdraw_subcommand(CLI::App* actionRoot) {
      auto withdraw = actionRoot->add_subcommand("withdraw", localized("Withdraw from owner's COM fund by transfering to owner's liquid token balance"));
      withdraw->add_option("owner",  owner_str,  localized("Account which owns the COM fund"))->required();
      withdraw->add_option("amount", amount_str, localized("Amount to be withdrawn from COM fund"))->required();
...
struct buycom_subcommand {
   string from_str;
   string amount_str;
   const name act_name{ N(buycom) };

   buycom_subcommand(CLI::App* actionRoot) {
      auto buycom = actionRoot->add_subcommand("buycom", localized("Buy COM using tokens in owner's COM fund"));
      buycom->add_option("from",   from_str,   localized("Account buying COM tokens"))->required();
      buycom->add_option("amount", amount_str, localized("Amount to be taken from COM fund and used in buying COM"))->required();
      add_standard_transaction_options(buycom, "from@active");
      buycom->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()
            ("from",   from_str)
            ("amount", amount_str);
         auto accountPermissions = get_account_permissions(tx_permission, {name(from_str), config::active_name});
...
struct lendcom_subcommand {
   string from_str;
   string amount_str;
   const name act_name1{ N(deposit) };
   const name act_name2{ N(buycom) };

   lendcom_subcommand(CLI::App* actionRoot) {
      auto lendcom = actionRoot->add_subcommand("lendcom", localized("Deposit tokens to COM fund and use the tokens to buy COM"));
      lendcom->add_option("from",   from_str,   localized("Account buying COM tokens"))->required();
      lendcom->add_option("amount", amount_str, localized("Amount of liquid tokens to be used in buying COM"))->required();
      add_standard_transaction_options(lendcom, "from@active");
      lendcom->set_callback([this] {
...
struct unstaketocom_subcommand {
   string owner_str;
   string receiver_str;
   string from_net_str;
   string from_cpu_str;
   const name act_name{ N(unstaketocom) };

   unstaketocom_subcommand(CLI::App* actionRoot) {
      auto unstaketocom = actionRoot->add_subcommand("unstaketocom", localized("Buy COM using staked tokens"));
      unstaketocom->add_option("owner",    owner_str,    localized("Account buying COM tokens"))->required();
      unstaketocom->add_option("receiver", receiver_str, localized("Account that tokens have been staked to"))->required();
      unstaketocom->add_option("from_net", from_net_str, localized("Amount to be unstaked from Net resources and used in COM purchase"))->required();
      unstaketocom->add_option("from_cpu", from_cpu_str, localized("Amount to be unstaked from CPU resources and used in COM purchase"))->required();
      add_standard_transaction_options(unstaketocom, "owner@active");
      unstaketocom->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()
            ("owner",    owner_str)
            ("receiver", receiver_str)
...
struct sellcom_subcommand {
   string from_str;
   string com_str;
   const name act_name{ N(sellcom) };

   sellcom_subcommand(CLI::App* actionRoot) {
      auto sellcom = actionRoot->add_subcommand("sellcom", localized("Sell COM tokens"));
      sellcom->add_option("from", from_str, localized("Account selling COM tokens"))->required();
      sellcom->add_option("com",  com_str,  localized("Amount of COM tokens to be sold"))->required();
      add_standard_transaction_options(sellcom, "from@active");
      sellcom->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()
            ("from", from_str)
            ("com",  com_str);
         auto accountPermissions = get_account_permissions(tx_permission, {name(from_str), config::active_name});
...
struct cancelcomorder_subcommand {
   string owner_str;
   const name act_name{ N(cnclcomorder) };

   cancelcomorder_subcommand(CLI::App* actionRoot) {
      auto cancelcomorder = actionRoot->add_subcommand("cancelcomorder", localized("Cancel queued COM sell order if one exists"));
      cancelcomorder->add_option("owner", owner_str, localized("Owner account of sell order"))->required();
      add_standard_transaction_options(cancelcomorder, "owner@active");
      cancelcomorder->set_callback([this] {
...
struct mvtosavings_subcommand {
   ...
   mvtosavings_subcommand(CLI::App* actionRoot) {
      auto mvtosavings = actionRoot->add_subcommand("mvtosavings", localized("Move COM tokens to savings bucket"));
      mvtosavings->add_option("owner", owner_str, localized("COM owner"))->required();
      mvtosavings->add_option("com",   com_str,   localized("Amount of COM to be moved to savings bucket"))->required();
...
struct mvfrsavings_subcommand {
   ...
   mvfrsavings_subcommand(CLI::App* actionRoot) {
      auto mvfrsavings = actionRoot->add_subcommand("mvfromsavings", localized("Move COM tokens out of savings bucket"));
      mvfrsavings->add_option("owner", owner_str, localized("COM owner"))->required();
      mvfrsavings->add_option("com",   com_str,   localized("Amount of COM to be moved out of savings bucket"))->required();
...
struct updatecom_subcommand {
   string owner_str;
   const name act_name{ N(updatecom) };

   updatecom_subcommand(CLI::App* actionRoot) {
      auto updatecom = actionRoot->add_subcommand("updatecom", localized("Update COM owner vote stake and vote weight"));
      updatecom->add_option("owner", owner_str, localized("COM owner"))->required();
      add_standard_transaction_options(updatecom, "owner@active");
      updatecom->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()("owner", owner_str);
         auto accountPermissions = get_account_permissions(tx_permission, {name(owner_str), config::active_name});
         send_actions({create_action(accountPermissions, config::system_account_name, act_name, act_payload)});
      });
...
struct consolidate_subcommand {
   ...
   consolidate_subcommand(CLI::App* actionRoot) {
      auto consolidate = actionRoot->add_subcommand("consolidate", localized("Consolidate COM maturity buckets into one that matures in 4 days"));
      consolidate->add_option("owner", owner_str, localized("COM owner"))->required();
...
struct comexec_subcommand {
   string user_str;
   string max_str;
   const name act_name{ N(comexec) };

   comexec_subcommand(CLI::App* actionRoot) {
      auto comexec = actionRoot->add_subcommand("comexec", localized("Perform COM maintenance by processing expired loans and unfilled sell orders"));
      comexec->add_option("user", user_str, localized("User executing the action"))->required();
      comexec->add_option("max",  max_str,  localized("Maximum number of CPU loans, Network loans, and sell orders to be processed"))->required();
      add_standard_transaction_options(comexec, "user@active");
      comexec->set_callback([this] {
            fc::variant act_payload = fc::mutable_variant_object()
               ("user", user_str)
               ("max",  max_str);
...
struct closecom_subcommand {
   string owner_str;
   const name act_name{ N(closecom) };

   closecom_subcommand(CLI::App* actionRoot) {
      auto closecom = actionRoot->add_subcommand("closecom", localized("Delete unused COM-related user table entries"));
      closecom->add_option("owner", owner_str, localized("COM owner"))->required();
      add_standard_transaction_options(closecom, "owner@active");
      closecom->set_callback([this] {
         fc::variant act_payload = fc::mutable_variant_object()("owner", owner_str);
         auto accountPermissions = get_account_permissions(tx_permission, {name(owner_str), config::active_name});
         send_actions({create_action(accountPermissions, config::system_account_name, act_name, act_payload)});
      });
...
void get_account( const string& accountName, const string& coresym, bool json_format ) {
   ...
   if (!json_format) {
      ...
      if( res.com_info.is_object() ) {
         auto& obj = res.com_info.get_object();
         asset vote_stake = asset::from_string( obj["vote_stake"].as_string() );
         asset com_balance = asset::from_string( obj["com_balance"].as_string() );
         std::cout << com_balance.get_symbol().name() << " balances: " << std::endl;
         std::cout << indent << std::left << std::setw(11)
                   << "balance:" << std::right << std::setw(18) << com_balance << std::endl;
         std::cout << indent << std::left << std::setw(11)
...
//                                ->check(CLI::ExistingFile);
   ...
   propose_action->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(arisen.msig), N(propose), variant_to_bin( N(arisen.msig), N(propose), args ) }});
   });
   ...
   review->set_callback([&] {
      ...
      std::map<arisen::account_name, std::pair<fc::time_point, vector<decltype(all_approvals)::iterator>>>  provided_approvers;
      ...
      if( show_approvals_in_multisig_review ) {
         ...
         if( new_multisig ) {
            for( auto& a : provided_approvers ) {
               ...
               if( rows4.empty() || rows4[0].get_object()["account"].as<arisen::name>() != a.first ) {
                  continue;
               }
   ...
   auto approve_or_unapprove = [&](const string& action) {
      ...
      send_actions({chain::action{accountPermissions, N(arisen.msig), name(action), variant_to_bin( N(arisen.msig), name(action), args ) }});
   };
   ...
   invalidate->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(arisen.msig), N(invalidate), variant_to_bin( N(arisen.msig), N(invalidate), args ) }});
   });
   ...
   cancel->set_callback([&]() {
      ...
      send_actions({chain::action{accountPermissions, N(arisen.msig), N(cancel), variant_to_bin( N(arisen.msig), N(cancel), args ) }});
      }
   );
   ...
   exec->set_callback([&] {
      ...
      send_actions({chain::action{accountPermissions, N(arisen.msig), N(exec), variant_to_bin( N(arisen.msig), N(exec), args ) }});
      }
   );
   ...
   auto com = system->add_subcommand("com", localized("Actions related to COM (the resource exchange)"));
   com->require_subcommand();
   auto deposit        = deposit_subcommand(com);
   auto withdraw       = withdraw_subcommand(com);
   auto buycom         = buycom_subcommand(com);
   auto lendcom        = lendcom_subcommand(com);
   auto unstaketocom   = unstaketocom_subcommand(com);
   auto sellcom        = sellcom_subcommand(com);
   auto cancelcomorder = cancelcomorder_subcommand(com);
   auto mvtosavings    = mvtosavings_subcommand(com);
   auto mvfromsavings  = mvfrsavings_subcommand(com);
   auto rentcpu        = rentcpu_subcommand(com);
   auto rentnet        = rentnet_subcommand(com);
   auto fundcpuloan    = fundcpuloan_subcommand(com);
   auto fundnetloan    = fundnetloan_subcommand(com);
   auto defcpuloan     = defcpuloan_subcommand(com);
   auto defnetloan     = defnetloan_subcommand(com);
   auto consolidate    = consolidate_subcommand(com);
   auto updatecom      = updatecom_subcommand(com);
   auto comexec        = comexec_subcommand(com);
   auto closecom       = closecom_subcommand(com);
