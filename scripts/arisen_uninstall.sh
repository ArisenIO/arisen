...
function usage() {
  ...
  -i DIR      Directory where arisen is installed)
...
if [[ $NONINTERACTIVE == false ]] && $FULL; then
   while true; do
      read -p "By specifying -f, removal of the arisen data directory will require a resync of data which can take days. Do you wish to proceed? (y/n) " PROCEED
      case $PROCEED in
...
export ARISEN_INSTALL_DIR=${INSTALL_LOCATION:-$ARISEN_INSTALL_DIR}

if [[ ! -d "${ARISEN_INSTALL_DIR}" ]]; then
   echo "[ARISEN installation ${COLOR_YELLOW}NOT${COLOR_NC} found in ${ARISEN_INSTALL_DIR}]"
else
   # As of 1.8.0, we're using a versioned directories under home: https://github.com/ARISENIO/arisen/issues/6940
      ...
      case $PROCEED in
         ...
         0 | true | [Yy]* )
            ...
            if $FULL; then
               ...
               [[ $ARCH == "Darwin" ]] && INSTALL_PATHS+=("${HOME}/Library/Application\ Support/arisen")
               [[ $ARCH != "Darwin" ]] && INSTALL_PATHS+=("${HOME}/.local/share/arisen")
            fi
            # Version < 1.8.0; Before we started using ~/arisen/1.8.x
...
if [[ $ARCH == "Darwin" ]]; then
   for package in $(cat scripts/arisen_build_darwin_deps | cut -d, -f1 2>/dev/null); do
      while true; do
