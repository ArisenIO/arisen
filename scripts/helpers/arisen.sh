...
function setup() {
    if $VERBOSE; then
        ...
        echo "ARISEN_INSTALL_DIR: ${ARISEN_INSTALL_DIR}"
...
function install-directory-prompt() {
    if [[ -z $INSTALL_LOCATION ]]; then
        ...
        while true; do
            [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}Do you wish to use the default location? ${ARISEN_INSTALL_DIR}? (y/n)${COLOR_NC}" && read -p " " PROCEED
            echo ""
            case $PROCEED in
                "" )
                echo "What would you like to do?";;
                ...
                break;;
                1 | false | [Nn]* )
                printf "Enter the desired installation location." && read -p " " ARISEN_INSTALL_DIR;
                export ARISEN_INSTALL_DIR;
                break;;
                * ) echo "Please type 'y' for yes or 'n' for no.";;
            esac
        done
    else
        # Support relative paths : https://github.com/arisenio/arisen/issues/7560
        [[ ! $INSTALL_LOCATION =~ ^\/ ]] && export INSTALL_LOCATION="${CURRENT_WORKING_DIR}/$INSTALL_LOCATION"
        export ARISEN_INSTALL_DIR="$INSTALL_LOCATION"
    fi
    . ./scripts/.build_vars
    echo "ARISEN will be installed to: ${ARISEN_INSTALL_DIR}"
}

function previous-install-prompt() {
  if [[ -d $ARISEN_INSTALL_DIR ]]; then
    echo "ARISEN has already been installed into ${ARISEN_INSTALL_DIR}... It's suggested that you arisen_uninstall.sh before re-running this script."
    while true; do
...
function ensure-compiler() {
    ...
    if $PIN_COMPILER || [[ -f $CLANG_ROOT/bin/clang++ ]]; then
        ...
        if [[ $CXX =~ "clang" ]] || [[ $COMPILER_TYPE =~ "clang" ]]; then
            ...
            if [[ $NO_CPP17 == false ]]; then # https://github.com/arisenio/arisen/issues/7402
                while true; do
                    echo "${COLOR_YELLOW}WARNING: Your GCC compiler ($CXX) is less performant than clang (https://github.com/arisenio/arisen/issues/7402). We suggest running the build script with -P or install your own clang and try again.${COLOR_NC}"
                    [[ $NONINTERACTIVE == false ]] && printf "${COLOR_YELLOW}Do you wish to proceed anyway? (y/n)?${COLOR_NC}" && read -p " " PROCEED
                    case $PROCEED in
                        "" ) echo "What would you like to do?";;
                        0 | true | [Yy]* ) break;;
...
function ensure-cmake() {
    ...
    if [[ ! -e "${CMAKE}" ]]; then
        ...
        && ./bootstrap --prefix=${ARISEN_INSTALL_DIR} \
        && make -j${JOBS} \
        && make install \
        && cd .. \
