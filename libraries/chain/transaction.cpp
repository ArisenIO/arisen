...
void deferred_transaction_generation_context::reflector_init() {
      ...
      RSN_ASSERT( sender != account_name(), ill_formed_deferred_transaction_generation_context,
...
{ try {
   ...
   for(const signature_type& sig : signatures) {
      ...
      RSN_ASSERT( now < deadline, tx_cpu_usage_exceeded, "transaction signature verification executed for too long ${time}us",
...
flat_multimap<uint16_t, transaction_extension> transaction::validate_and_extract_extensions()const {
   ...
   for( size_t i = 0; i < transaction_extensions.size(); ++i ) {
      ...
      RSN_ASSERT( id >= id_type_lower_bound, invalid_transaction_extension,
      ...
      RSN_ASSERT( match, invalid_transaction_extension,
                  "Transaction extension with id type ${id} is not supported",
                  ("id", id)
      );

      if( match->enforce_unique ) {
         RSN_ASSERT( i == 0 || id > id_type_lower_bound, invalid_transaction_extension,
...
void packed_transaction::reflector_init()
{
   ...
   RSN_ASSERT( unpacked_trx.expiration == time_point_sec(), tx_decompression_error, "packed_transaction already unpacked" );
   local_unpack_transaction({});
   local_unpack_context_free_data();
}
...
{
   try {
      RSN_ASSERT(unpacked_trx.context_free_data.empty(), tx_decompression_error, "packed_transaction.context_free_data not empty");
      switch( compression ) {
         case compression_type::none:
