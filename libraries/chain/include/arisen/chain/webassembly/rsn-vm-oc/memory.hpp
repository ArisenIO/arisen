...
#include <arisen/chain/webassembly/rsn-vm-oc/gs_seg_helpers.h>
...
class memory {
      static constexpr uint64_t wasm_memory_size                  = arisen::chain::wasm_constraints::maximum_linear_memory;
      ...
      static constexpr uint64_t mutable_global_size               = 8u  * arisen::chain::wasm_constraints::maximum_mutable_globals/4u;
      static constexpr uint64_t table_size                        = 16u * arisen::chain::wasm_constraints::maximum_table_elements;
      static constexpr size_t   wcb_allowance                     = 512u;
      static_assert(sizeof(control_block) <= wcb_allowance, "RSN VM OC memory doesn't set aside enough memory for control block");
   ...
   public:
      ...
      static_assert(-cb_offset == RSN_VM_OC_CONTROL_BLOCK_OFFSET, "RSN VM OC control block offset has slid out of place somehow");
      static_assert(stride == RSN_VM_OC_MEMORY_STRIDE, "RSN VM OC memory stride has slid out of place somehow");
...
#define OFFSET_OF_CONTROL_BLOCK_MEMBER(M) (-(int)arisen::chain::rsnvmoc::memory::cb_offset + (int)offsetof(arisen::chain::rsnvmoc::control_block, M))
#define OFFSET_OF_FIRST_INTRINSIC ((int)-arisen::chain::rsnvmoc::memory::first_intrinsic_offset)