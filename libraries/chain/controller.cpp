...
struct controller_impl {
   ...
   void pop_block() {
      ...
      if( !prev ) {
         RSN_ASSERT( fork_db.root()->id == head->header.previous, block_validate_exception, "attempt to pop beyond last irreversible block" );
         prev = fork_db.root();
      }
      ...
      if ( read_mode == db_read_mode::SPECULATIVE ) {
         RSN_ASSERT( head->block, block_validate_exception, "attempting to pop a block that was sparsely loaded from a snapshot");
      }
   ...
   inline void set_activation_handler() {
      ...
      RSN_ASSERT( res.second, misc_exception, "attempting to set activation handler twice" );
   }
   ...
   :rnh(),
    ...
    wasmif( cfg.wasm_runtime, cfg.rsnvmoc_tierup, db, cfg.state_dir, cfg.rsnvmoc_config ),
    resource_limits( db ),
    authorization( s, db ),
...
*/
   ...
   void log_irreversible() {
      RSN_ASSERT( fork_db.root(), fork_database_exception, "fork database not properly initialized" );
      ...
      if( log_head ) {
         RSN_ASSERT( root_id == blog.head_id(), fork_database_exception, "fork database root does not match block log head" );
      } else {
         RSN_ASSERT( fork_db.root()->block_num == lib_num, fork_database_exception,
   ...
   void replay(std::function<bool()> shutdown) {
      ...
      if( start_block_num <= blog_head->block_num() ) {
         ...
         if( pending_head->block_num < head->block_num || head->block_num < fork_db.root()->block_num ) {
            ...
            RSN_ASSERT( new_root, fork_database_exception, "unexpected error: could not find new LIB in fork database" );
   ...
   void startup(std::function<bool()> shutdown, const snapshot_reader_ptr& snapshot) {
      RSN_ASSERT( snapshot, snapshot_exception, "No snapshot reader provided" );
      ...
      try {
         ...
         if( blog.head() ) {
            ...
            RSN_ASSERT( lib_num > 0, snapshot_exception,
   ...
   void startup(std::function<bool()> shutdown, const genesis_state& genesis) {
      RSN_ASSERT( db.revision() < 1, database_exception, "This version of controller::startup only works with a fresh state database." );
      const auto& genesis_chain_id = genesis.compute_chain_id();
      RSN_ASSERT( genesis_chain_id == chain_id, chain_id_type_exception,
      ...
      if( blog.head() ) {
         RSN_ASSERT( blog.first_block_num() == 1, block_log_exception,
   ...
   void startup(std::function<bool()> shutdown) {
      RSN_ASSERT( db.revision() >= 1, database_exception, "This version of controller::startup does not work with a fresh state database." );
      RSN_ASSERT( fork_db.head(), fork_database_exception, "No existing fork database despite existing chain state. Replay required." );

      uint32_t lib_num = fork_db.root()->block_num;
      auto first_block_num = blog.first_block_num();
      if( blog.head() ) {
         RSN_ASSERT( first_block_num <= lib_num && lib_num <= blog.head()->block_num(),
                     block_log_exception,
   ...
   static auto validate_db_version( const chainbase::database& db ) {
      ...
      RSN_ASSERT(header_idx.begin() != header_idx.end(), bad_database_version_exception,
   ...
   void init(std::function<bool()> shutdown) {
...

      {
         ...
         RSN_ASSERT( state_chain_id == chain_id, chain_id_type_exception,
      ...
      RSN_ASSERT( db.revision() >= head->block_num, fork_database_exception,
      ...
      if( read_mode == db_read_mode::IRREVERSIBLE ) {
         ...
         RSN_ASSERT( itr == rbi.end() || itr->blocknum == lib_num + 1, reversible_blocks_exception,
         ...
         RSN_ASSERT( head->block_num <= last_block_num, reversible_blocks_exception,
         ...
         if( ritr != rbi.rend()
            ...
            RSN_ASSERT( rbitr != rbi.end(), reversible_blocks_exception, "pending head block not found in reversible blocks");
            auto rev_id = rbitr->get_block_id();
            RSN_ASSERT( rev_id == pending_head->id,
   ...
   void read_from_snapshot( const snapshot_reader_ptr& snapshot, uint32_t blog_start, uint32_t blog_end ) {
      ...
      { /// load and upgrade the block header state
         ...
         RSN_ASSERT( blog_start <= (snapshot_head_block + 1) && snapshot_head_block <= blog_end,
                     block_log_exception,
      ...
      controller_index_set::walk_indices([this, &snapshot, &header]( auto utils ){
         ...
         if (std::is_same<value_t, global_property_object>::value) {
            ...
            if (std::clamp(header.version, v2::minimum_version, v2::maximum_version) == header.version ) {
               ...
               RSN_ASSERT( genesis, snapshot_exception,
      ...
      RSN_ASSERT( gpo.chain_id == chain_id, chain_id_type_exception,
   ...
   void create_native_account( const fc::time_point& initial_timestamp, account_name name, const authority& owner, const authority& active, bool is_privileged = false ) {
      db.create<account_object>([&](auto& a) {
         ...
         if( name == config::system_account_name ) {
            // The initial arisen ABI value affects consensus; see  https://github.com/arisenio/arisen/issues/7794
   ...
   { try {
      ...
      RSN_ASSERT( !validating || explicit_billed_cpu_time, transaction_exception, "validating requires explicit billing" );
      ...
      if ( !subjective ) {
         ...
         if( !validating ) {
            ...
            RSN_ASSERT( !explicit_billed_cpu_time || (cpu_time_to_bill_us == limited_cpu_time_to_bill_us),
                                           ...
                                           bool explicit_billed_cpu_time )
   {
      ...
      try {
         ...
         if( !explicit_billed_cpu_time ) {
            fc::microseconds already_consumed_time( RSN_PERCENT(sig_cpu_usage.count(), conf.sig_cpu_bill_pct) );
                     ...
                     const optional<block_id_type>& producer_block_id )
   {
      ...
      if ( read_mode == db_read_mode::SPECULATIVE || pending->_block_status != controller::block_status::incomplete )
      {
         ...
         if( new_protocol_feature_activations.size() > 0 ) {
            ...
            for( const auto& feature_digest : new_protocol_feature_activations ) {
               ...
               if( res.second ) {
                  ...
                  RSN_ASSERT( !f.preactivation_required, protocol_feature_exception,
                              "attempted to activate protocol feature without prior required preactivation: ${digest}",
                              ("digest", feature_digest)
                  );
               } else {
                  RSN_ASSERT( !res.first->second, block_validate_exception,
         ...
         RSN_ASSERT( handled_all_preactivated_features, block_validate_exception,
         ...
         )
         {
            ...
            RSN_ASSERT( gpo.proposed_schedule.version == pbhs.active_schedule_version + 1,
   ...
   void finalize_block()
   {
      ...
      RSN_ASSERT( pending->_block_stage.contains<building_block>(), block_validate_exception, "already called finalize_block");
      ...
      resource_limits.set_block_parameters(
         ...
         {RSN_PERCENT(chain_config.max_block_net_usage, chain_config.target_block_net_usage_pct), chain_config.max_block_net_usage, config::block_size_average_window_ms / config::block_interval_ms, config::maximum_elastic_resource_multiplier, {99, 100}, {1000, 999}}
      );
   ...
   void commit_block( bool add_to_fork_db ) {
      ...
      try {
         RSN_ASSERT( pending->_block_stage.contains<completed_block>(), block_validate_exception,
         ...
         if( add_to_fork_db ) {
            ...
            head = fork_db.head();
            RSN_ASSERT( bsp == head, fork_database_exception, "committed block did not become the new head in fork database");
         }
      ...
      for( auto itr = new_protocol_features.begin(); itr != new_protocol_features.end(); ++itr ) {
         ...
         switch( status ) {
            case protocol_feature_set::recognized_t::unrecognized:
               RSN_THROW( protocol_feature_exception,
            ...
            break;
            default:
               RSN_THROW( protocol_feature_exception, "unexpected recognized_t status" );
            break;
         }

         RSN_ASSERT( currently_activated_protocol_features.find( f ) == currently_activated_protocol_features.end(),
         ...
         RSN_ASSERT( pfs.validate_dependencies( f, dependency_checker ), protocol_feature_exception,
...
#define RSN_REPORT(DESC,A,B) \
   if( A != B ) { \
      elog("${desc}: ${bv} != ${abv}", ("desc", DESC)("bv", A)("abv", B)); \
   }

      RSN_REPORT( "timestamp", b.timestamp, ab.timestamp )
      RSN_REPORT( "producer", b.producer, ab.producer )
      RSN_REPORT( "confirmed", b.confirmed, ab.confirmed )
      RSN_REPORT( "previous", b.previous, ab.previous )
      RSN_REPORT( "transaction_mroot", b.transaction_mroot, ab.transaction_mroot )
      RSN_REPORT( "action_mroot", b.action_mroot, ab.action_mroot )
      RSN_REPORT( "schedule_version", b.schedule_version, ab.schedule_version )
      RSN_REPORT( "new_producers", b.new_producers, ab.new_producers )
      RSN_REPORT( "header_extensions", b.header_extensions, ab.header_extensions )
...
#undef RSN_REPORT
   ...
   { try {
      try {
         ...
         for( const auto& receipt : b->transactions ) {
            ...
            RSN_ASSERT( trx_receipts.size() > 0,
                        block_validate_exception, "expected a receipt",
                        ("block", *b)("expected_receipt", receipt)
                      );
            RSN_ASSERT( trx_receipts.size() == num_pending_receipts + 1,
         ...
         if( producer_block_id != ab._id ) {
            ...
            RSN_ASSERT( producer_block_id == ab._id, block_validate_exception, "Block ID does not match",
   ...
   std::future<block_state_ptr> create_block_state_future( const signed_block_ptr& b ) {
      RSN_ASSERT( b, block_validate_exception, "null block" );

      auto id = b->id();

      // no reason for a block_state if fork_db already knows about block
      auto existing = fork_db.get_block( id );
      RSN_ASSERT( !existing, fork_database_exception, "we already know about this block: ${id}", ("id", id) );

      auto prev = fork_db.get_block_header( b->previous );
      RSN_ASSERT( prev, unlinkable_block_exception,
      ...
      return async_thread_pool( thread_pool.get_executor(), [b, prev, control=this]() {
         ...
         RSN_ASSERT( b->transaction_mroot == trx_mroot, block_validate_exception,
   ...
   void replay_push_block( const signed_block_ptr& b, controller::block_status s ) {
      ...
      try {
         ...
         RSN_ASSERT( (s == controller::block_status::irreversible || s == controller::block_status::validated),
         ...
         if( s == controller::block_status::irreversible ) {
            ...
            RSN_ASSERT( read_mode != db_read_mode::IRREVERSIBLE, block_validate_exception,
   ...
   void check_actor_list( const flat_set<account_name>& actors )const {
      ...
      if( conf.actor_whitelist.size() > 0 ) {
         ...
         RSN_ASSERT( is_subset,  actor_whitelist_exception,
         ...
         RSN_ASSERT( !intersects, actor_blacklist_exception,
...
void controller::preactivate_feature( const digest_type& feature_digest ) {
   ...
   switch( status ) {
      case protocol_feature_set::recognized_t::unrecognized:
         if( is_producing_block() ) {
            RSN_THROW( subjective_block_production_exception,
                       "protocol feature with digest '${digest}' is unrecognized", ("digest", feature_digest) );
         } else {
            RSN_THROW( protocol_feature_bad_block_exception,
                       "protocol feature with digest '${digest}' is unrecognized", ("digest", feature_digest) );
         }
      break;
      case protocol_feature_set::recognized_t::disabled:
      ...
      default:
         if( is_producing_block() ) {
            RSN_THROW( subjective_block_production_exception, "unexpected recognized_t status" );
         } else {
            RSN_THROW( protocol_feature_bad_block_exception, "unexpected recognized_t status" );
         }
      break;
   }
   ...
   // Even if they don't recognize the feature, the producer could change their aos code to treat it like an
   ...
   RSN_ASSERT( !is_protocol_feature_activated( feature_digest ),
               protocol_feature_exception,
   ...
   RSN_ASSERT( std::find( pso.preactivated_protocol_features.begin(),
   ...
   RSN_ASSERT( pfs.validate_dependencies( feature_digest, dependency_checker ),
               protocol_feature_exception,
...
void controller::start_block( block_timestamp_type when, uint16_t confirm_block_count )
{
   ...
   RSN_ASSERT( !my->pending, block_validate_exception, "pending block already exists" );
...
transaction_trace_ptr controller::push_transaction( const transaction_metadata_ptr& trx, fc::time_point deadline,
   ...
   validate_db_available_size();
   RSN_ASSERT( get_read_mode() != db_read_mode::IRREVERSIBLE, transaction_type_exception, "push transaction not allowed in irreversible mode" );
...
transaction_trace_ptr controller::push_scheduled_transaction( const transaction_id_type& trxid, fc::time_point deadline,
                                                              uint32_t billed_cpu_time_us, bool explicit_billed_cpu_time )
{
   RSN_ASSERT( get_read_mode() != db_read_mode::IRREVERSIBLE, transaction_type_exception, "push scheduled transaction not allowed in irreversible mode" );
   validate_db_available_size();
...
block_id_type controller::get_block_id_for_num( uint32_t block_num )const { try {
   ...
   RSN_ASSERT( BOOST_LIKELY( id != block_id_type() ), unknown_block_exception,
...
void controller::write_snapshot( const snapshot_writer_ptr& snapshot ) const {
   RSN_ASSERT( !my->pending, block_validate_exception, "cannot take a consistent snapshot with a pending block" );
...
void controller::set_greylist_limit( uint32_t limit ) {
   RSN_ASSERT( 0 < limit && limit <= chain::config::maximum_elastic_resource_multiplier,
               misc_exception,
