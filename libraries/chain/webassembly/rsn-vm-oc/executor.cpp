...
static intrinsic arisen_exit_intrinsic("env.arisen_exit", IR::FunctionType::get(IR::ResultType::none,{IR::ValueType::i32}), (void*)&arisen_exit,
  boost::hana::index_if(intrinsic_table, ::boost::hana::equal.to(BOOST_HANA_STRING("env.arisen_exit"))).value()
);
...
void executor::execute(const code_descriptor& code, const memory& mem, apply_context& context) {
   ...
   cb->current_call_depth_remaining = arisen::chain::wasm_constraints::maximum_call_depth+2;
