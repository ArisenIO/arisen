...
class currency_tester : public TESTER {
   public:
      ...
      static const name arisen_token;
};

const name currency_tester::arisen_token = N(arisen.token);
...
BOOST_FIXTURE_TEST_CASE(test_symbol, TESTER) try {
   ...
   {
      symbol sys(4, "RSN");
      BOOST_REQUIRE_EQUAL(SY(4,RSN), sys.value());
      BOOST_REQUIRE_EQUAL("4,RSN", sys.to_string());
      BOOST_REQUIRE_EQUAL("RSN", sys.name());
