...
BOOST_AUTO_TEST_CASE( webauthn_producer ) { try {
   ...
   BOOST_CHECK_THROW(
      ...
      arisen::chain::unactivated_key_type
   );
...
BOOST_AUTO_TEST_CASE( webauthn_create_account ) { try {
   ...
   BOOST_CHECK_THROW(c.push_transaction(trx), arisen::chain::unactivated_key_type);
...
BOOST_AUTO_TEST_CASE( webauthn_recover_key ) { try {
   ...
   BOOST_CHECK_THROW(c.push_transaction(trx), arisen::chain::unactivated_signature_type);
...
BOOST_AUTO_TEST_CASE( producer_schedule_change_extension_test ) { try {
   ...
   { // ensure producer_schedule_change_extension is rejected
      ...
      bad_block->producer_signature = remote.get_private_key(N(arisen), "active").sign(sig_digest);
...
BOOST_AUTO_TEST_CASE( wtmsig_block_signing_inflight_legacy_test ) { try {
   ...
   c.preactivate_protocol_features( {*d} );
   vector<legacy::producer_key> sched = {{N(arisen), c.get_public_key(N(arisen), "bsk")}};
   ...
   c.block_signing_private_keys.emplace(get_public_key(N(arisen), "bsk"), get_private_key(N(arisen), "bsk"));
   c.produce_block();
