...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   chain.link_authority(name("alice"), name("arisen"), name("spending"), name("reqauth"));
   ...
   BOOST_CHECK_THROW( chain.link_authority(name("alice"), name("arisen"), name("spending"), name("reqauth")), action_validate_exception);

   // Unlink alice with arisen reqauth
   chain.unlink_authority(name("alice"), name("arisen"), name("reqauth"));
   ...
   chain.link_authority(name("alice"), name("arisen"), name("scud"));
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority(name("alice"), name("arisen"), name("first"), name("reqauth"));
...
try {
   ...
   chain.create_account(name("arisen.test1"));

   // Creating account with arisen. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account(name("arisen.test2"), name("joe")), action_validate_exception,
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( name("alice"), name("arisen"), name("arisen.any"), name("reqauth") );
   chain.link_authority( name("bob"), name("arisen"), name("arisen.any"), name("reqauth") );
