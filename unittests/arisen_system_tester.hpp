...
public:
   ...
   arisen_system_tester(Lambda setup) {
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( name("arisen") ) );
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
            get_balance(name("arisen")) + get_balance(name("arisen.ramfee")) + get_balance(name("arisen.stake")) + get_balance(name("arisen.ram")) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(arisen.token), act, N(accounts), name(symbol(CORE_SYMBOL).to_symbol_code().value) );
   ...
   fc::variant get_stats( const string& symbolname ) {
      ...
      vector<char> data = get_row_by_account( N(arisen.token), name(symbol_code), N(stat), name(symbol_code) );
   ...
   abi_serializer initialize_multisig() {
      abi_serializer msig_abi_ser;
      {
         ...
         BOOST_REQUIRE_EQUAL( success(), buyram( name("arisen"), name("arisen.msig"), core_from_string("5000.0000") ) );
         produce_block();
         ...
         set_code( N(arisen.msig), contracts::arisen_msig_wasm() );
         set_abi( N(arisen.msig), contracts::arisen_msig_abi().data() );
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( name("arisen"), name("alice1111111"), core_from_string("650000000.0000"), name("arisen") );
