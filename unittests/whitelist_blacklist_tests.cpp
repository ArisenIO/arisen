...
BOOST_AUTO_TEST_CASE( actor_blacklist_inline_deferred ) { try {
   ...
   auto auth = authority(arisen::testing::base_tester::get_public_key(name("alice"), "active"));
   ...
   auth = authority(arisen::testing::base_tester::get_public_key(name("bob"), "active"));
   auth.accounts.push_back( permission_level_weight{{N(alice), config::arisen_code_name}, 1} );
   ...
   auth = authority(arisen::testing::base_tester::get_public_key(name("charlie"), "active"));
   ...
   auto log_trxs = [&](std::tuple<const transaction_trace_ptr&, const signed_transaction&> x) {
      ...
      if( act.account == N(arisen) && act.name == N(onblock) ) return;
