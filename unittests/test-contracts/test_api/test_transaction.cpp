...
struct test_action_action {
   static arisen::name get_account() {
      return arisen::name{ACCOUNT};
   }

   static arisen::name get_name() {
      return arisen::name{NAME};
   }
...
void test_transaction::assert_false_error_handler( const arisen::transaction& dtrx ) {
   ...
   arisen_assert( dtrx.actions[0].account == "testapi"_n, "transaction has wrong code" );
   arisen_assert( dtrx.actions[0].name.value == WASM_TEST_ACTION("test_action", "assert_false"), "transaction has wrong name" );
   arisen_assert( dtrx.actions[0].authorization.size() == 1, "action should only have one authorization" );
   arisen_assert( dtrx.actions[0].authorization[0].actor == "testapi"_n, "action's authorization has wrong actor" );
   arisen_assert( dtrx.actions[0].authorization[0].permission == "active"_n, "action's authorization has wrong permission" );
}
...
void test_transaction::stateful_api() {
   char buf[4] = {1};
   db_store_i64( arisen::name{"testtrans"}.value, arisen::name{"table"}.value, arisen::name{"testtrans"}.value, 0, buf, 4 );
}
...
void test_transaction::repeat_deferred_transaction( uint64_t receiver, uint64_t code, uint64_t action ) {
   ...
   trx.send( sender_id, arisen::name{receiver} );
}
