...
class Cluster(object):
    ...
    __BiosPort=8788
    __LauncherCmdArr=[]
    __bootlog="arisen-ignition-wd/bootlog.txt"

    # pylint: disable=too-many-arguments
    # walletd [True|False] Is awalletd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=12518, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="RSNtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet awalletd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: rsn server host
        port: rsn server port
        walletHost: rsn wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures rsn mongo plugin
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
               totalProducers=None, sharedProducers=0, extraAosArgs=" --http-max-response-time-ms 990000 ", useBiosBootFile=True, specificExtraAosArgs=None, onlySetProds=False,
        ...
        loadSystemContract: indicate whether the arisen.system contract should be loaded (setting this to False causes useBiosBootFile to be treated as False)
        """
        ...
        aosArgs="--max-transaction-time -1 --abi-serializer-max-time-ms 990000 --filter-on \"*\" --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
        ...
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            aosArgs += " --plugin arisen::producer_api_plugin"
        ...
        if nodes is None or startedNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.RsnServerName, startedNodes, len(nodes)))
            return False
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on rsn server port %d" % (
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on rsn node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        receiving transferAmount*n RSN and forwarding x-transferAmount funds. Transfer actions are spread round-robin across the cluster to vaidate system cohesiveness."""
    ...
    def bios_bootstrap(self, biosNode, totalNodes, pfSetupPolicy, silent=False):
        ...
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        env = {
            "BIOS_CONTRACT_PATH": "unittests/contracts/old_versions/v1.6.0-rc3/arisen.bios",
            "FEATURE_DIGESTS": ""
        }
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            env["BIOS_CONTRACT_PATH"] = "unittests/contracts/old_versions/v1.7.0-develop-preactivate_feature/arisen.bios"
        ...
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull, env=env):
            if not silent: Utils.Print("Launcher failed to shut down rsn cluster.")
            return None
        ...
        if not self.walletMgr.importKey(arisenAccount, ignWallet):
        ...
        for name, keys in producerKeys.items():
            data="{\"from\":\"arisen\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (name, initialFunds, "init arisen transfer")
    ...
    def bootstrap(self, biosNode, totalNodes, prodCount, totalProducers, pfSetupPolicy, onlyBios=False, onlySetProds=False, loadSystemContract=True):
        """Create 'prodCount' init accounts and deposits 10000000000 RSN in each. If prodCount is -1 will initialize all possible producers.
        ...
        action="create"
        data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\"}" % (arisenAccount.name, CORE_SYMBOL)
        ...
        if loadSystemContract:
            ...
            opts="--permission %s@active" % (arisenAccount.name)
            trans=biosNode.pushMessage(arisenAccount.name, action, data, opts)
    ...
    def pgrepRsnServers(timeout=None):
        cmd=Utils.pgrepCmd(Utils.RsnServerName)
    ...
    def discoverLocalNode(self, nodeNum, psOut=None, timeout=None):
        ...
        if psOut is None:
            Utils.Print("ERROR: No nodes discovered.")
            return None
        pattern=Cluster.pgrepRsnServerPattern(nodeNum)
    ...
    def relaunchRsnInstances(self, cachePopen=False):
    ...
    def killall(self, silent=True, allInstances=False):
        ...
        if allInstances:
            # ocassionally the launcher cannot kill the rsn server
            cmd="pkill -9 %s" % (Utils.RsnServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down rsn cluster.")
