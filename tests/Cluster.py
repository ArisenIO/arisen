...
class Cluster(object):
    ...
    def launch(self, pnodes=1, unstartedNodes=0, totalNodes=1, prodCount=1, topo="mesh", delay=1, onlyBios=False, dontBootstrap=False,
        ...
        specificExtraNodeosArgs: dictionary of arguments to pass to a specific node (via --specific-num and
                                 --specific-nodeos flags on launcher), example: { "5" : "--plugin arisen::test_control_api_plugin" }
        ...
        if not self.walletd:
            nodeosArgs += " --plugin arisen::wallet_api_plugin"
        if self.enableMongo:
            nodeosArgs += " --plugin arisen::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        ...
        if PFSetupPolicy.hasPreactivateFeature(pfSetupPolicy):
            nodeosArgs += " --plugin arisen::producer_api_plugin"
