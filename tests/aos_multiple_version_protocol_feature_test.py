...
###############################################################
# aos_multiple_version_protocol_feature_test
#
# Test for verifying that older versions of aos can work with newer versions of aos.
#
###############################################################
...
def restartNode(node: Node, nodeId, chainArg=None, addSwapFlags=None, aosPath=None):
    if not node.killed:
        node.kill(signal.SIGTERM)
    isRelaunchSuccess = node.relaunch(nodeId, chainArg, addSwapFlags=addSwapFlags,
                                      timeout=5, cachePopen=True, aosPath=aosPath)
...
try:
    ...
    assert cluster.launch(pnodes=4, totalNodes=4, prodCount=1, totalProducers=4,
                          extraAosArgs=" --plugin arisen::producer_api_plugin ",
    ...
    # Finally, when we restart the 4th node with the version of aos that supports protocol feature,
    ...
    oldNode.relaunch(oldNodeId, chainArg="--import-reversible-blocks {}".format(portableRevBlkPath), timeout=1, aosPath="programs/aos/aos")
    os.remove(portableRevBlkPath)

    restartNode(oldNode, oldNodeId, chainArg="--replay", aosPath="programs/aos/aos")
