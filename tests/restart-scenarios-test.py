...
#
# Tests restart scenarios for aos.  Uses "-c" flag to indicate "replay" (--replay-blockchain), "resync"
# (--delete-all-blocks), "hardReplay"(--hard-replay-blockchain), and "none" to indicate what kind of restart flag should
# be used. This is one of the only test that actually verify that aos terminates with a good exit status.
#
###############################################################
...
try:
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful=testSuccessful, killRsnInstances=killRsnInstances, killWallet=killRsnInstances, keepLogs=keepLogs, cleanRun=killAll, dumpErrorDetails=dumpErrorDetails)
