...
# find aos
[[ $(git --version) ]] && cd "$(git rev-parse --show-toplevel)/build/programs/aos" || cd "$(dirname "${BASH_SOURCE[0]}")/../programs/aos"
if [[ ! -f aos ]]; then
...
./aos --extract-build-info build-info.json 1>/dev/null 2>/dev/null
