...
SCRIPT_LOCATION="scripts/arisen_build.sh"
TEST_LABEL="[arisen_build]"
...
@test "${TEST_LABEL} > Testing Arguments & Options" {
    ...
    run bash -c "./arisen_build.sh -y -P"
    ...
    # lack of -i
    [[ ! -z $(echo "${output}" | grep "ARISEN_INSTALL_DIR: ${HOME}/arisen/${ARISEN_VERSION}") ]] || exit
    ## -o
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -o Debug -P"
    [[ ! -z $(echo "${output}" | grep "CMAKE_BUILD_TYPE: Debug") ]] || exit
    ## -s
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -s RSN2 -P"
    [[ ! -z $(echo "${output}" | grep "CORE_SYMBOL_NAME: RSN2") ]] || exit
    ## -b
    run bash -c "printf \"y\ny\nn\nn\n\" | ./$SCRIPT_LOCATION -b /test -P"
    [[ ! -z $(echo "${output}" | grep "BOOST_LOCATION: /test") ]] || exit
    ## -i
    run bash -c "printf \"y\ny\nn\nn\n\"| ./$SCRIPT_LOCATION -i /NEWPATH -P"
    [[ ! -z $(echo "${output}" | grep "ARISEN_INSTALL_DIR: /NEWPATH") ]] || exit
    [[ ! -z $(echo "${output}" | grep "TEMP_DIR: ${HOME}/tmp") ]] || exit
        ...
        [[ ! -z $(echo "${output}" | grep "ARISEN_INSTALL_DIR: $TEMP_DIR/NEWPATH") ]] || exit
