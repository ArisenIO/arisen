...
class TestHelper(object):
    ...
    def parse_args(includeArgs, applicationSpecificArgs=AppArgs()):
        ...
        if "--wallet-host" in includeArgs:
            parser.add_argument("--wallet-host", type=str, help="%s host" % Utils.rsnWalletName,
                                     default=TestHelper.LOCAL_HOST)
        if "--wallet-port" in includeArgs:
            parser.add_argument("--wallet-port", type=int, help="%s port" % Utils.rsnWalletName,
        ...
        if "--clean-run" in includeArgs:
            parser.add_argument("--clean-run", help="Kill all aos and klrsn instances", action='store_true')
        if "--sanity-test" in includeArgs:
            parser.add_argument("--sanity-test", help="Validates aos and klrsn are in path and can be started up.", action='store_true')
    ...
    def shutdown(cluster, walletMgr, testSuccessful=True, killrsnInstances=True, killWallet=True, keepLogs=False, cleanRun=True, dumpErrorDetails=False):
        ...
        if not testSuccessful and dumpErrorDetails:
            ...
            psOut=Cluster.pgreprsnServers(timeout=60)
