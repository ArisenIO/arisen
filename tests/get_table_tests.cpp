...
BOOST_FIXTURE_TEST_CASE( get_scope_test, TESTER ) try {
   ...
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, arisen::chain::asset::from_string("999.0000 SYS") );
   }
   produce_blocks(1);
...
BOOST_FIXTURE_TEST_CASE( get_table_test, TESTER ) try {
   ...
   // issue
   for (account_name a: accs) {
      issue_tokens( *this, config::system_account_name, a, arisen::chain::asset::from_string("10000.0000 SYS") );
   }
   produce_blocks(1);
...
BOOST_FIXTURE_TEST_CASE( get_table_by_seckey_test, TESTER ) try {
   ...
   bidname(N(inita), N(com), arisen::chain::asset::from_string("10.0000 SYS"));
   bidname(N(initb), N(org), arisen::chain::asset::from_string("11.0000 SYS"));
   bidname(N(initc), N(io), arisen::chain::asset::from_string("12.0000 SYS"));
   bidname(N(initd), N(html), arisen::chain::asset::from_string("14.0000 SYS"));
   produce_blocks(1);
