...
FC_REFLECT(arisen::detail::txn_test_gen_status, (status));
...
#define INVOKE_V_R_R_R(api_handle, call_name, in_param0, in_param1, in_param2) \
     ...
     arisen::detail::txn_test_gen_status result = { status };
   ...
   [this](string, string body, url_response_callback cb) mutable { \
      ...
      auto result_handler = [times_called{std::move(times_called)}, cb, body](const fc::exception_ptr& e) mutable {\
         ...
         if (e) {\
            ...
            cb(http_response_code, fc::variant(arisen::detail::txn_test_gen_empty())); \
         }\
      };\
      INVOKE \
   }\
...
struct txn_test_gen_plugin_impl {
   ...
   fc::optional<arisen::chain::named_thread_pool>        thread_pool;
   ...
   void create_test_accounts(const std::string& init_name, const std::string& init_priv_key, const std::function<void(const fc::exception_ptr&)>& next) {
      ...
      try {
         ...
         abi_def currency_abi_def = fc::json::from_string(contracts::arisen_token_abi().data()).as<abi_def>();

         controller& cc = app().get_plugin<chain_plugin>().chain();
         auto chainid = app().get_plugin<chain_plugin>().get_chain_id();
         auto abi_serializer_max_time = app().get_plugin<chain_plugin>().get_abi_serializer_max_time();

         abi_serializer arisen_token_serializer{fc::json::from_string(contracts::arisen_token_abi().data()).as<abi_def>(), abi_serializer_max_time};
         ...
         //set newaccountT contract to arisen.token & initialize it
         {
            signed_transaction trx;

            vector<uint8_t> wasm = contracts::arisen_token_wasm();
...

            {
               setabi handler;
               handler.account = newaccountT;
               handler.abi = fc::raw::pack(json::from_string(contracts::arisen_token_abi().data()).as<abi_def>());
               trx.actions.emplace_back( vector<chain::permission_level>{{newaccountT,name("active")}}, handler);
            }

            {
               ...
               act.data = arisen_token_serializer.variant_to_binary("create",
            ...
            }
            {
               ...
               act.name = N(issue);
               act.authorization = vector<permission_level>{{newaccountT,config::active_name}};
               act.data = arisen_token_serializer.variant_to_binary("issue",
            ...
            }
            {
               ...
               act.name = N(transfer);
               act.authorization = vector<permission_level>{{newaccountT,config::active_name}};
               act.data = arisen_token_serializer.variant_to_binary("transfer",
...
void txn_test_gen_plugin::plugin_initialize(const variables_map& options) {
   try {
      ...
      my->newaccountA = arisen::chain::name(thread_pool_account_prefix + "a");
      my->newaccountB = arisen::chain::name(thread_pool_account_prefix + "b");
      my->newaccountT = arisen::chain::name(thread_pool_account_prefix + "t");
