...
void chain_apis::read_write::validate() const {
   RSN_ASSERT( !db.in_immutable_mode(), missing_chain_api_plugin_exception, "Not allowed, node in read-only mode" );
}
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      ...
      t_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple( config::system_account_name, config::system_account_name, N(bexbal) ));
      if (t_id != nullptr) {
         const auto &idx = d.get_index<key_value_index, by_scope_primary>();
         auto it = idx.find(boost::make_tuple( t_id->id, params.account_name.to_uint64_t() ));
         if( it != idx.end() ) {
            vector<char> data;
            copy_inline_row(*it, data);
            result.bex_info = abis.binary_to_variant( "bex_balance", data, abi_serializer_max_time, shorten_abi_errors );
         }
      }
   }
   return result;
