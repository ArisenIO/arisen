...
std::ostream& operator<<(std::ostream& osm, arisen::chain::db_read_mode m) {
   if ( m == arisen::chain::db_read_mode::SPECULATIVE ) {
      ...
      osm << "head";
   } else if ( m == arisen::chain::db_read_mode::READ_ONLY ) { // deprecated
      osm << "read-only";
...
void chain_plugin::set_program_options(options_description& cli, options_description& cfg)
{
   cfg.add_options()
         ...
         ("wasm-runtime", bpo::value<arisen::chain::wasm_interface::vm_type>()->value_name("runtime")->notifier([](const auto& vm){
#ifndef ARISEN_RSN_VM_OC_DEVELOPER
            //throwing an exception here (like RSN_ASSERT) is just gobbled up with a "Failed to initialize" error :(
            if(vm == wasm_interface::vm_type::rsn_vm_oc) {
               elog("RSN VM OC is a tier-up compiler and works in conjunction with the configured base WASM runtime. Enable RSN VM OC via 'rsn-vm-oc-enable' option");
               RSN_ASSERT(false, plugin_exception, "");
            }
#endif
         ...
         ("rsn-vm-oc-cache-size-mb", bpo::value<uint64_t>()->default_value(rsnvmoc::config().cache_size / (1024u*1024u)), "Maximum size (in MiB) of the RSN VM OC code cache")
         ("rsn-vm-oc-compile-threads", bpo::value<uint64_t>()->default_value(1u)->notifier([](const auto t) {
               if(t == 0) {
                  elog("rsn-vm-oc-compile-threads must be set to a non-zero value");
                  RSN_ASSERT(false, plugin_exception, "");
               }
         }), "Number of threads to use for RSN VM OC tier-up")
         ("rsn-vm-oc-enable", bpo::bool_switch(), "Enable RSN VM OC tier-up runtime")
#endif
         ;
...
protocol_feature_set initialize_protocol_features( const fc::path& p, bool populate_missing_builtins = true ) {
   ...
   if( fc::exists( p ) ) {
      RSN_ASSERT( fc::is_directory( p ), plugin_exception,
   ...
   if( directory_exists ) {
      for( directory_iterator enditr, itr{p}; itr != enditr; ++itr ) {
         ...
         RSN_ASSERT( res.second, plugin_exception,
   ...
   auto output_protocol_feature = [&p]( const builtin_protocol_feature& f, const digest_type& feature_digest ) {
      ...
      RSN_ASSERT( !fc::exists( file_path ), plugin_exception,
...
void chain_plugin::plugin_initialize(const variables_map& options) {
   ...
   try {
      try {
         ...
         elog( "ARISEN_ROOT_KEY ('${root_key}') is invalid. Recompile with a valid public key.",
      ...
      if( options.at( "print-build-info" ).as<bool>() || options.count( "extract-build-info") ) {
         ...
         if( options.count( "extract-build-info") ) {
            ...
            RSN_ASSERT( fc::json::save_to_file( chainbase::environment(), p, true ), misc_exception,
         ...
         RSN_THROW( node_management_success, "reported build environment information" );
      }
      ...
      if( options.count( "chain-threads" )) {
         ...
         RSN_ASSERT( my->chain_config->thread_pool_size > 0, plugin_config_exception,
      ...
      RSN_ASSERT( my->chain_config->sig_cpu_bill_pct >= 0 && my->chain_config->sig_cpu_bill_pct <= 100, plugin_config_exception,
      ...
      if( options.count( "extract-genesis-json" ) || options.at( "print-genesis-json" ).as<bool>()) {
         ...
         if( options.count( "extract-genesis-json" )) {
            ...
            RSN_ASSERT( fc::json::save_to_file( *gs, p, true ),
                        misc_exception,
      ...
      if (options.count( "snapshot" )) {
         ...
         RSN_ASSERT( fc::exists(*my->snapshot_path), plugin_config_exception,
         ...
         RSN_ASSERT( !fc::is_regular_file(shared_mem_path),
                 plugin_config_exception,
         ...
         if( fc::is_regular_file( my->blocks_dir / "blocks.log" )) {
            ...
            if( block_log_genesis ) {
               ...
               RSN_ASSERT( *chain_id == block_log_chain_id,
                           plugin_config_exception,
         ...
         if( fc::is_regular_file( my->blocks_dir / "blocks.log" ) ) {
            ...
            if( chain_id ) {
               RSN_ASSERT( *block_log_chain_id == *chain_id, block_log_exception,
         ...
         if( options.count( "genesis-json" ) ) {
            ...
            if( block_log_genesis ) {
               RSN_ASSERT( *block_log_genesis == provided_genesis, plugin_config_exception,
               ...
               if( chain_id ) {
                  RSN_ASSERT( provided_genesis_chain_id == *chain_id, plugin_config_exception,
                  ...
                  if( block_log_chain_id ) {
                     RSN_ASSERT( provided_genesis_chain_id == *block_log_chain_id, plugin_config_exception,
                                 "Genesis state, provided via command line arguments, has a chain ID (${provided_genesis_chain_id}) "
         ...
         if( !chain_id ) {
            if( my->genesis ) {
               ...
               RSN_ASSERT( !block_log_chain_id, plugin_config_exception,
...
#ifdef ARISEN_RSN_VM_OC_RUNTIME_ENABLED
      if( options.count("rsn-vm-oc-cache-size-mb") )
         my->chain_config->rsnvmoc_config.cache_size = options.at( "rsn-vm-oc-cache-size-mb" ).as<uint64_t>() * 1024u * 1024u;
      if( options.count("rsn-vm-oc-compile-threads") )
         my->chain_config->rsnvmoc_config.threads = options.at("rsn-vm-oc-compile-threads").as<uint64_t>();
      if( options["rsn-vm-oc-enable"].as<bool>() )
         my->chain_config->rsnvmoc_tierup = true;
#endif
...
{ try {
   RSN_ASSERT( my->chain_config->read_mode != db_read_mode::IRREVERSIBLE || !accept_transactions(), plugin_config_exception,
...
void chain_apis::read_write::validate() const {
   RSN_ASSERT( api_accept_transactions, missing_chain_api_plugin_exception,
...
void chain_plugin::handle_guard_exception(const chain::guard_exception& e) {
   ...
   elog("database chain::guard_exception, quiting..."); // log string searched for in: tests/aos_under_min_avail_ram.py
   // quit the app
   app().quit();
}
...
double convert_to_type(const string& str, const string& desc) {
   ...
   RSN_ASSERT( !std::isnan(val), chain::contract_table_query_exception,
...
string convert_to_string(const chain::key256_t& source, const string& key_type, const string& encode_type, const string& desc) {
   try {
      ...
      RSN_ASSERT( false, chain_type_exception, "Incompatible key_type and encode_type for key256_t next_key" );
...
fc::variant read_only::get_block(const read_only::get_block_params& params) const {
   ...
   RSN_ASSERT( !params.block_num_or_id.empty() && params.block_num_or_id.size() <= 64,
...
read_only::get_account_results read_only::get_account( const get_account_params& params )const {
   ...
   if( abi_serializer::to_abi(code_account.abi, abi) ) {
      ...
      t_id = d.find<chain::table_id_object, chain::by_code_scope_table>(boost::make_tuple( config::system_account_name, config::system_account_name, N(combal) ));
      if (t_id != nullptr) {
         const auto &idx = d.get_index<key_value_index, by_scope_primary>();
         auto it = idx.find(boost::make_tuple( t_id->id, params.account_name.to_uint64_t() ));
         if( it != idx.end() ) {
            vector<char> data;
            copy_inline_row(*it, data);
            result.com_info = abis.binary_to_variant( "com_balance", data, abi_serializer_max_time, shorten_abi_errors );
         }
      }
   }
   return result;
