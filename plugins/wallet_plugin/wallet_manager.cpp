...
void wallet_manager::own_and_use_wallet(const string& name, std::unique_ptr<wallet_api>&& wallet) {
   if(wallets.find(name) != wallets.end())
      RSN_THROW(wallet_exception, "Tried to use wallet name that already exists.");
   wallets.emplace(name, std::move(wallet));
}
   ...
   t->async_wait([t, this](const boost::system::error_code& /*ec*/)
   {
      ...
      if(ec != boost::system::error_code()) {
         if(rc.type() == bfs::file_not_found) {
            appbase::app().quit();
            RSN_THROW(wallet_exception, "Lock file removed while awalletd still running.  Terminating.");
         }
      }
...
void wallet_manager::initialize_lock() {
   //This is technically somewhat racy in here -- if multiple awalletd are in this function at once.
   ...
   if(!wallet_dir_lock->try_lock()) {
      ...
      RSN_THROW(wallet_exception, "Failed to lock access to wallet directory; is another awalletd running?");
   }
